...
class AdbCrypto {
    ...
    companion object {
        ...
        const val KEY_LENGTH_BITS: Int = 2048

        /** The ADB RSA key length in bytes  */
        const val KEY_LENGTH_BYTES: Int = KEY_LENGTH_BITS / 8

        /** The ADB RSA key length in words  */
        const val KEY_LENGTH_WORDS: Int = KEY_LENGTH_BYTES / 4

        /** The RSA signature padding as an int array  */
        val SIGNATURE_PADDING_AS_INT: IntArray = intArrayOf(
        ...
        var SIGNATURE_PADDING: ByteArray = ByteArray(SIGNATURE_PADDING_AS_INT.size)

        init {
            for (i in SIGNATURE_PADDING.indices) SIGNATURE_PADDING[i] = SIGNATURE_PADDING_AS_INT[i].toByte()
        }
